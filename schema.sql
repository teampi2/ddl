...
CREATE TABLE IF NOT EXISTS `mydb`.`schools` (
  ...
  CONSTRAINT `fk_schools_account_id`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT chk_schools_cnpj_format CHECK (`cnpj` REGEXP '^[0-9]{2}\\.[0-9]{3}\\.[0-9]{3}/[0-9]{4}-[0-9]{2}$'),
...
CREATE TABLE IF NOT EXISTS `mydb`.`classes` (
  ...
  CONSTRAINT `fk_classes_account_id`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT chk_classes_academic_year_format CHECK (`academic_year` REGEXP '^[0-9]{4}(\.[1-2])?$'))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `mydb`.`attendances` (
  ...
  CONSTRAINT `fk_attendances_lesson_id`
    ...
    ON DELETE CASCADE
    ON UPDATE NO ACTION,
  CONSTRAINT chk_attendances_is_present_format CHECK (is_present IN (0, 1)))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `mydb`.`verification_codes` (
  ...
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  INDEX `verification_codes_code_idx` (`code` ASC) VISIBLE,
  CONSTRAINT chk_verification_codes_email_format CHECK (`email` LIKE '%@%.%'))
ENGINE = InnoDB;
